...
class DualPINN(torch.nn.Module):
    ...
    def calc_bc(self, x_bc:torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        return torch.concat((y_bc_pred[:,:3], y_bc_pred[:,3:]), dim=1)
    ...
    def calc_ic(self, x_ic:torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        return torch.concat((y_ic_pred[:,:3], y_ic_pred[:,3:]), dim=1)
    ...
    def loss_fn(self,
        ...
        return bc_loss*self.bc_weight + self.alignment_weight*alignment_loss + self.mom_weight*mom_loss + self.div_weight*div_loss + self.ic_weight*ic_loss
    ...
    def eval_losses(self,
        ...
        tot_loss_val = bc_loss + mom_loss + div_loss + alignment_loss + ic_loss
        return mom_loss, div_loss, y_loss, bc_loss, ic_loss, alignment_loss, tot_loss_val
