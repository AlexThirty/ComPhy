...
parser.add_argument('--inc_weight', default=1e-1, type=float, help='Weight for the divergence loss')
...
parser.add_argument('--mom_weight', default=3e-2, type=float, help='Weight for the PINN loss')
...
parser.add_argument('--name', default='triplepinn', type=str, help='Experiment name')
...
from model_params import triplepinn_params
params = triplepinn_params[mode]
...
model = TriplePINN(
    ...
    mom_hidden_units=[units for _ in range(layers)],
...
checkpoint_dir = 'saved_models/triplepinn_checkpoints'
start_epoch = 0
if args.restart:
    ...
    if checkpoints:
        ...
        model.load_state_dict(torch.load(os.path.join(checkpoint_dir, latest_checkpoint), weights_only=True))
...
def train_loop(epochs:int,
    ...
    for epoch in range(epochs):
        ...
        if epoch % 50 == 0:
            if not os.path.exists('saved_models/triplepinn_checkpoints'):
                os.makedirs('saved_models/triplepinn_checkpoints')
            torch.save(model.state_dict(), f'saved_models/triplepinn_checkpoints/triplepinn_{mode}_{epoch}.pt')
...
torch.save(model.state_dict(), f'saved_models/triplepinn_{mode}_{weight_type}.pt')

save_dir = f'results_triplepinn_{weight_type}'
