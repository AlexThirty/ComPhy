...
parser.add_argument('--mode', default='ncl', type=str, help='Mode: -1 for PINN learning, 0 for derivative learning, 1 for output learning')
...
parser.add_argument('--units', default=512, type=int, help='Number of units per layer in the network')
parser.add_argument('--weight_type', default='static', type=str, help='Type of weighting: static or grad')
...
checkpoint_dir = 'saved_models/ncl_checkpoints'
start_epoch = 0
if os.path.exists(checkpoint_dir) and not restart:
    ...
    for epoch in range(epochs):
        if epoch % 5 == 0 and epoch > 0:
            ...
            if weight_type == 'grad':
                ...
                loss_sums = ic_grad_norm + mom_grad_norm + inc_grad_norm
                ...
                print(f'Adaptive Weights - INIT: {model.init_weight}, INC: {model.inc_weight}, MOM: {model.mom_weight}')
        ...
        for step, (pde_data, init_data, sol_data) in enumerate(zip(internal_dataloader, cycle(initial_dataloader), cycle(solution_dataloader))):
            ...
            if (step_prefix+step) % print_every == 0 and step>0:
                with torch.no_grad():
                    _, mom_loss, inc_loss, y_loss, init_loss_val, tot_loss_val = model.eval_losses(
                    ...
                    print(f'Step: {step_prefix+step}, Mom loss: {mom_loss}, inc loss: {inc_loss}, y loss: {y_loss}')
                    print(f'init loss: {init_loss_val}, Total loss: {tot_loss_val}')
        ...
        if epoch % 100 == 0:
            if not os.path.exists('saved_models/ncl_checkpoints'):
                os.makedirs('saved_models/ncl_checkpoints')
            torch.save(model.state_dict(), f'saved_models/ncl_checkpoints/ncl_{epoch}.pt')
            print('Checkpoint saved')

        print(f'Test Mom loss: {mom_loss_test}, Test inc loss: {inc_loss_test}, Test y loss: {y_loss_test}')
...
torch.save(model.state_dict(), f'saved_models/ncl_{weight_type}.pt')

save_dir = f'results_ncl_{weight_type}'
...
train_losses = np.vstack((step_list, mom_losses, inc_losses, y_losses, init_losses, tot_losses)).T
test_losses = np.vstack((step_list_test, mom_losses_test, inc_losses_test, y_losses_test, init_losses_test, tot_losses_test, time_test)).T

np.save(f'{save_dir}/ncl_train_losses.npy', train_losses)
np.save(f'{save_dir}/ncl_test_losses.npy', test_losses)
...
plt.savefig(f'{save_dir}/ncl_training_losses.png')
...
plt.legend()
plt.grid(True, which="both", ls="--")
plt.savefig(f'{save_dir}/ncl_testing_losses.png')