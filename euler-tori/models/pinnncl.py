...
class PINNNCL(torch.nn.Module):
    ...
    def loss_fn(self, 
        ...
        if alignment_mode == 'DERL':
            alignment_loss = self.loss_container(Du_div - Dy_inc, torch.zeros_like(Dy_inc))
        ...
        return init_loss*self.init_weight + self.mom_weight*mom_loss + self.alignment_weight*alignment_loss + self.inc_weight*inc_loss
    ...
    def calc_inc_loss(self, x_pde:torch.Tensor):
        Dy = vmap(jacrev(self.forward_single))(x_pde)[:,4:]
        inc_pde = vmap(torch.trace)(Dy[:,:,1:3])
        inc_loss = self.loss_container(inc_pde, torch.zeros_like(inc_pde))
        return inc_loss
