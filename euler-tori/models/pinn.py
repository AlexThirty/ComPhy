...
class PINN(torch.nn.Module):
    ...
    def loss_fn(self, 
        ...
        div_pde = vmap(torch.trace)(Dy_pred[:,:3])
        ...
        return init_loss*self.init_weight + self.mom_weight*mom_loss + self.inc_weight*inc_loss + self.div_weight*div_loss
    ...
    def eval_losses(self, 
        ...
        y_loss = self.loss_container(y_sol_pred[:,:-1], y_sol)
        
        div_pde = vmap(torch.trace)(Dy_pred[:,:3])
        div_loss = self.loss_container(div_pde, torch.zeros_like(div_pde))
        
        tot_loss_val = inc_loss + mom_loss + init_loss + div_loss
        #print(f'Step: {step}, F_loss: {F_loss_val}, div_loss: {div_loss_val}, bc_loss: {bc_loss_val}, init_loss: {init_loss_val}')
        return step, div_loss, mom_loss, inc_loss, y_loss, init_loss, tot_loss_val
    
    
    def forward_u_single(self, tx: torch.Tensor):
        out = self.forward_single(tx, return_final=True)
