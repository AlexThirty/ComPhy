...
class DualPINN_NCL(torch.nn.Module):
    def __init__(self,
                 alignment_mode:str,
        ...
        self.alignment_loss_container = torch.nn.HuberLoss(reduction='mean') 
        ...
        self.positivizer = SoftAbs().to(self.device)
        ...
        total_params = ind_params + mom_params + state_params
    ...
    def forward(self, tx, return_final=False):
        ...
        def gauss_A_matrix(x:torch.Tensor):
            ...
            gauss_out = self.ind_net(gauss_in)[-1:]
        ...
        gauss_out = torch.einsum('...ii', gauss_fun(tx.reshape((-1,3)))[:,:,:,1:])
        
        non_div_out = self.ind_net(self.embed(tx.reshape((-1,3))))
        if non_div_out.ndim == 1:
            non_div_out = non_div_out.unsqueeze(0)
        non_div_out = non_div_out[:, :-1].reshape((-1,2))

        ind_out = torch.cat([non_div_out, gauss_out], dim=1)
        # Get the predictions
        tx = self.embed(tx)
        ...
        #return torch.cat((self.positivizer(mom_out[:,:1]), mom_out[:,1:3], self.positivizer(mom_out[:,3:4]), mom_out[:,4:6], state_out[:,0:2], self.positivizer(state_out[:,2:3]), state_out[:,3:5], ind_out.reshape((-1,4))), dim=1).squeeze(0)
        return torch.cat((mom_out, state_out, ind_out), dim=1).squeeze(0)
    ...
    def calc_alignment_loss(
        ...
        if self.alignment_mode ==  'DERL':
            ...
            alignment_loss = self.alignment_loss_container(Dy_ind_pred - torch.cat([Dy_mom_pred[:,1:3], Dy_mom_pred[:,4:6]], dim=1), torch.zeros_like(Dy_ind_pred))
            alignment_loss = alignment_loss + self.alignment_loss_container(y_ind_pred - torch.cat([y_mom_pred[:,1:3], y_mom_pred[:,4:6]], dim=1), torch.zeros_like(y_ind_pred))
            alignment_loss = alignment_loss + self.alignment_loss_container(Dy_state_pred - Dy_mom_pred[:,1:], torch.zeros_like(Dy_state_pred))
            alignment_loss = alignment_loss + self.alignment_loss_container(y_state_pred - y_mom_pred[:,1:], torch.zeros_like(y_state_pred))
            #alignment_loss += self.alignment_loss_container(Dy_ind_pred - torch.cat([Dy_state_pred[:,0:2], Dy_state_pred[:,3:5]], dim=1), torch.zeros_like(Dy_ind_pred))
            #alignment_loss += self.alignment_loss_container(y_ind_pred - torch.cat([y_state_pred[:,0:2], y_state_pred[:,3:5]], dim=1), torch.zeros_like(y_ind_pred))
    ...
    def calc_ic_loss(self,
        ...
        return self.loss_container(y_ic_pred[:,:6], y_ic) + self.loss_container(y_ic_pred[:,6:11], y_ic[:,1:]) + self.loss_container(y_ic_pred[:,11:], torch.column_stack([y_ic[:,1:3], y_ic[:,4:6]]))
    ...
    def eval_state_loss(self, x_pde:torch.Tensor, **kwargs) -> torch.Tensor:
        y_pred = self.forward(x_pde)[:,1:6]
        Dy_pred = vmap(jacrev(self.forward))(x_pde)[:,1:6]
    ...
    def loss_fn(self,
        ...
        tot_loss = self.cont_weight*cont_loss + self.mom_weight*mom_loss + self.state_weight*state_loss +\
            self.ind_weight*ind_loss + self.ic_weight*ic_loss + self.alignment_weight*alignment_loss
