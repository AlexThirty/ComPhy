...
class TriplePINN_NCL(torch.nn.Module):
    def __init__(self,
                 alignment_mode:str,
    ...
    def forward(self, tx, return_final=False):
        ...
        cont_out = self.cont_net(tx).reshape((-1,3))
        if return_final:
            return mom_out
    ...
    def calc_alignment_loss(
        ...
        if self.alignment_mode ==  'DERL':
            ...
            alignment_loss += self.alignment_loss_container(Dy_cont_pred - Dy_mom_pred[:,:3], torch.zeros_like(Dy_cont_pred))
            ...
            alignment_loss += self.alignment_loss_container(y_cont_pred - y_mom_pred[:,:3], torch.zeros_like(y_cont_pred))
    ...
    def calc_ic_loss(self,
        ...
        return self.loss_container(y_mom_pred, y_ic) + self.loss_container(y_state_pred, y_ic[:,1:]) +\
            self.loss_container(y_ind_pred, torch.cat([y_ic[:,1:3], y_ic[:,4:6]], dim=1)) + self.loss_container(y_cont_pred, y_ic[:,:3])
    ...
    def calc_cont(
        ...
        y_pred = self.forward(x_pde)[:,15:18]
        Dy_pred = vmap(jacrev(self.forward))(x_pde)[:,15:18]
